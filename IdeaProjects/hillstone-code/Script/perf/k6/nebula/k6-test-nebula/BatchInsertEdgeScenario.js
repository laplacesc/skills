...
var graph_option = {
  ...
  csv_path: "target/data/test_data/social_network/dynamic/person_likes_comment.csv",
  csv_delimiter: "|",
  csv_with_header: true,
  output: "output/20241226203336/output500_BatchInsertEdgeScenario.csv",
  username: "root",
  password: "nebula",
  max_size: 2000,
};
...
export default function (data) {
  let ngql = 'INSERT EDGE LIKES (creationDate) VALUES ';
  let batches = [];
  let batchSize = 100;
  // batch size 100
  for (let i = 0; i < batchSize; i++) {
    ...
    let value =  "{0}->{1}:(datetime(\"{2}\"))".format(d);
    batches.push(value);
  };
